...
//       const response = await axios.get(`${API_URL}/submissions/${id}/onedrive-files`, {
//         headers: { 'Authorization': `Bearer ${token}` }
//       })
...
//     // ✅ FIX: Open in popup to avoid redirect
//     const width = 600;
//     const height = 700;
...
//       try {
//         const statusResponse = await axios.get(`${API_URL}/onedrive/status`, {
//           headers: { 'Authorization': `Bearer ${token}` }
//         })
...
//               className="bg-primary-600 hover:bg-primary-700 text-white text-sm py-1.5 px-4 rounded-lg flex items-center gap-2"
//             >
...
//                           <span className="font-medium">₹{parseFloat(line.amount).toFixed(0)}</span>
//                         </div>
//                       ))}
...
//             <div className="bg-white rounded-xl shadow-sm p-6 border border-purple-200 bg-purple-50">
...
//               <div className="bg-white rounded-xl shadow-sm p-6 border border-blue-200 bg-blue-50">
//                 <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
//                   <Cloud className="w-5 h-5 text-blue-500" />
//                   Upload to OneDrive
//                 </h2>
//                 <p className="text-sm text-gray-600 mb-4">
//                   Upload all documents and computation to OneDrive for secure storage.
//                 </p>
...
//                     className="w-full bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white font-medium py-2.5 px-4 rounded-lg flex items-center justify-center text-sm"
//                   >
...
//                   <button
//                     onClick={handleConnectOneDrive}
//                     className="w-full bg-primary-600 hover:bg-primary-700 text-white font-medium py-2.5 px-4 rounded-lg flex items-center justify-center text-sm"
//                   >
//                     <Cloud className="w-4 h-4 mr-2" />
...
//                         target="_blank"
//                         rel="noopener noreferrer"
//                         className="text-sm text-blue-600 hover:text-blue-800 flex items-center gap-1 flex-shrink-0 ml-4"
//                       >
...
//             {/* 3. Status Actions - ONLY Approve, Reject, Refresh */}
//             <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
...
import { 
  ...
  ExternalLink, Upload as UploadIcon, FileCheck, RefreshCw, Cloud,
  Check, X, FileSpreadsheet, FileImage, FileVideo, FileAudio, FileArchive,
  Info, AlertCircle, PlusCircle, Download, Trash2, Share2
} from 'lucide-react'
...
const SubmissionDetail = () => {
  ...
  const handleConnectOneDrive = () => {
    ...
    // Clean up if popup is closed without connecting
                  ...
                  {onedriveFiles.map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-blue-50 rounded-lg border border-blue-100 hover:bg-blue-100 transition-colors">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm text-blue-600 hover:text-blue-800 flex items-center gap-1 flex-shrink-0 ml-4 transition-colors"
                      >
                        <Eye className="w-4 h-4" />
                        View
                      </a>
